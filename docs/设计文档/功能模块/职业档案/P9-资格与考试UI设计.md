...
| v1.7 | 2026-08-19 | AI | **P9-B 代码落地 + 发布区同步**：`ITab_Pawn_Career` SubTabs 追加 Qualification + switch 分发；新增 partial `ITab_Pawn_Career.Qualification.cs`（职称阶梯/申报条件 7 行/流程操作按钮（顺序门控）/考试任务卡/评审卡）；新增 `Dialog_QualificationFlow`（报名实践考试/论文选题·引用·完成/答辩召集·评分，D-D2 范式、D-D1 自动委员）；新增 `QualificationFlowService` 编排（CanApplyExam/ApplyForPracticalExam/SubmitTheoryExam/StartThesis/CiteThesisBook/CompleteThesis/StartDefense/GradeDefense，顺序门控+按档隔离+复用 RecordExamProduced 采集）；中英 +9 键（916=916 一致）；编译 0 错 0 警 + 单测 174 过/3 跳/0 失败；DLL 重建，发布区（Assemblies/Defs/Languages/Textures/About）就位 |
